...
The directory is built as shown here:
    ...
    content/drive/MyDrive/work
      ...
      |-resource
          ...
          |-face_detector.xml 
          |-gan_input_images
The first part of the model works on the face_detection.zip data's train set (this file is unzipped in the input folder) and exports it's output to the gan_input images folder.
...
## changes of plan since the first submission, and other notes to MileStone2:
  ...
  * The first part of the model only operates on the test set of the model as it is proven to be enough for the GAN (we're planning to use this for evaluation only).
